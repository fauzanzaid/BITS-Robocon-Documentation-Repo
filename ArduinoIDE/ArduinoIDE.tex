...
\section{Arduino sketch basics}
	...
	\subsection{Some code!}
		...
		These are two \textbf{functions}. The \inlncd{setup} function is run once when the Arduino is powered up or reset. The \inlncd{loop} runs repeatedly until the Arduino is powered down.
...
\section{Arduino functions}
	...
	\subsection{Pins}
		...
		\item \inlncd{pinMode}: Set the mode of the pin before writing digital \texttt{HIGH} or \texttt{LOW} to it. Takes two arguments, first is the pin number, the second is the mode. Pin mode can be \texttt{INPUT}, \texttt{OUTPUT} or \texttt{INPUT\_PULLUP}.
